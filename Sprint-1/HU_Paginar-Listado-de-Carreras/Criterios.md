...
1. **C1:** Existen suficientes carreras registradas que requieren múltiples páginas
...
1. **A1:** Mostrar controles de paginación en la parte inferior de la tabla de carreras
...
10. **A10:** Cargar y mostrar carreras correspondientes a la página seleccionada
...
14. **A14:** Ocultar controles de paginación cuando todas las carreras caben en una página
...
|-------|------------------|---------------|---------------|---------|
| **R1** | N | - | - | Ocultar controles de paginación (todas las carreras en una página) |
...
**Escenario 1 – Visualizar controles de paginación con múltiples páginas de carreras**

**Dado que** estoy autenticado como Autoridad en el sistema **Y** estoy en la pantalla "Gestión de Carreras" **Y** existen suficientes carreras registradas que requieren múltiples páginas para mostrarlas todas,  
**cuando** se carga la vista inicial del listado de carreras,  
...
**Escenario 2 – Ocultar controles cuando todas las carreras caben en una página**

**Dado que** estoy en la pantalla "Gestión de Carreras" **Y** el número total de carreras registradas es igual o menor al tamaño de página definido,  
**cuando** se carga el listado de carreras,  
**entonces** se muestran todas las carreras en una sola vista **Y** no se muestran controles de paginación en la parte inferior **Y** no aparecen botones "< Previous" ni "Next >" **Y** no se muestran números de página **Y** la tabla presenta todas las carreras disponibles sin necesidad de navegación adicional.

---

**Escenario 3 – Navegar a la siguiente página usando botón "Next"**

**Dado que** estoy en la pantalla "Gestión de Carreras" **Y** estoy visualizando la primera página de carreras **Y** existen más páginas disponibles **Y** el botón "Next >" está habilitado,  
**cuando** hago clic en el botón "Next >",  
**entonces** la tabla se actualiza mostrando las carreras de la página 2 **Y** la página actual se marca visualmente como "2" **Y** la tabla se actualiza sin recargar la página completa **Y** el botón "< Previous" se habilita porque ya no estoy en la primera página **Y** el botón "Next >" permanece habilitado si existen más páginas después de la 2 **Y** se mantiene visible la estructura de la tabla con todas sus columnas: "Código", "Nombre", "Facultad", "Coordinador", "Acciones" **Y** la transición es fluida y sin pérdida de contexto.

---

**Escenario 4 – Navegar a la página anterior usando botón "Previous"**

**Dado que** estoy en la pantalla "Gestión de Carreras" **Y** estoy visualizando la página 2 o superior **Y** el botón "< Previous" está habilitado,  
**cuando** hago clic en el botón "< Previous",  
**entonces** la tabla se actualiza mostrando las carreras de la página anterior **Y** la página actual se actualiza visualmente (de "2" a "1", o de "3" a "2") **Y** la tabla se actualiza sin recargar la página completa **Y** si navego de vuelta a la página 1, el botón "< Previous" se deshabilita **Y** el botón "Next >" permanece habilitado **Y** se mantienen todos los datos y estructura de la tabla con las carreras correspondientes.

---

**Escenario 5 – Navegar directamente a una página específica**

**Dado que** estoy en la pantalla "Gestión de Carreras" **Y** se muestran controles de paginación con múltiples números de página (ej: "1 2 3"),  
**cuando** hago clic en un número de página específico (ej: hago clic en "3"),  
**entonces** la tabla se actualiza mostrando las carreras correspondientes a la página 3 **Y** el número "3" se resalta o marca visualmente como página actual **Y** el número "1" pierde el resaltado de página actual **Y** la tabla se actualiza sin recargar la página completa **Y** el botón "< Previous" está habilitado porque no estoy en la primera página **Y** el botón "Next >" se habilita o deshabilita según si existen más páginas después de la 3 **Y** la navegación es inmediata al hacer clic.

---

**Escenario 6 – Deshabilitar botón "Next" en la última página**

**Dado que** estoy en la pantalla "Gestión de Carreras" **Y** estoy navegando por las páginas de carreras,  
**cuando** llego a la última página disponible (ej: página 3 de 3) mediante navegación o clic directo,  
**entonces** el número de la última página se resalta como página actual **Y** el botón "Next >" se deshabilita o muestra visualmente como inactivo **Y** el botón "< Previous" permanece habilitado porque no estoy en la primera página **Y** no es posible hacer clic en "Next >" para avanzar más **Y** se muestran todas las carreras restantes de la última página **Y** los controles de paginación permanecen visibles pero con "Next" inactivo.

---

**Escenario 7 – Mantener filtros aplicados al cambiar de página**

**Dado que** estoy en la pantalla "Gestión de Carreras" **Y** he aplicado filtros (ej: búsqueda por "Ingeniería" o filtro de facultad "Facultad de Ingeniería de Sistemas") **Y** los resultados filtrados requieren múltiples páginas **Y** estoy en la página 1 de los resultados filtrados,  
**cuando** hago clic en "Next >" o en un número de página diferente,  
**entonces** la tabla muestra la siguiente página de resultados filtrados **Y** se mantienen aplicados todos los filtros activos (búsqueda y/o dropdowns de facultad y estado) **Y** solo se muestran carreras que cumplen con los criterios de filtrado **Y** el campo de búsqueda conserva el texto ingresado **Y** los dropdowns de filtro conservan los valores seleccionados **Y** la paginación funciona solo sobre el conjunto de datos filtrados, no sobre todas las carreras **Y** el número total de páginas se ajusta según los resultados filtrados.

---

**Escenario 8 – Actualizar controles de paginación cuando los filtros reducen resultados**

**Dado que** estoy en la pantalla "Gestión de Carreras" **Y** tengo múltiples páginas de carreras sin filtros,  
**cuando** aplico un filtro o búsqueda que reduce significativamente los resultados (ej: búsqueda que devuelve solo 2 carreras),  
...
1. **Valida el propósito central de la HU:** La Historia de Usuario establece que la Autoridad quiere "navegar por el listado de carreras en bloques manejables para gestionar grandes volúmenes de información eficientemente". El Escenario 3 valida directamente la navegación secuencial hacia adelante, que es el flujo más natural y frecuente de uso de paginación.

2. **Representa el "happy path" principal de paginación:** Es el comportamiento más común y esperado:
   - Las autoridades naturalmente comienzan en la página 1
   - La progresión natural es avanzar hacia adelante para revisar más carreras
   - Es el patrón de navegación más intuitivo y frecuente
   - Refleja el flujo de revisión secuencial de la oferta académica

3. **Valida la funcionalidad core de gestión de grandes volúmenes:** Sin navegación hacia adelante efectiva:
   - La autoridad queda atrapada en la primera página
   - No puede acceder al resto del catálogo de carreras
   - Se pierde completamente el valor de la paginación
   - No se puede gestionar eficientemente el volumen total de información académica
...
4. **Verifica comportamiento dinámico crítico:** El escenario valida múltiples aspectos técnicos esenciales:
   ...
   - Preservación de todas las columnas (Código, Nombre, Facultad, Coordinador, Acciones)

5. **Mayor frecuencia de uso que otras navegaciones:** En la gestión de oferta académica:
   ...
   - Las autoridades tienden a revisar el catálogo de carreras progresivamente
...
7. **Impacto directo en eficiencia de gestión (objetivo de la HU):** La navegación fluida con Next permite:
   - Revisar sistemáticamente toda la oferta académica de la EPN
   - Gestionar información en bloques manejables (como indica la HU)
   - Mantener contexto mientras se explora el catálogo completo de carreras
   - Evitar sobrecarga cognitiva de ver decenas de carreras simultáneamente

8. **Validación de experiencia de usuario esperada:** Las autoridades académicas esperan que:
   - Next las lleve a la página siguiente inmediatamente
...
9. **Precedencia operativa en gestión académica:** Las autoridades típicamente:
   - Revisan carreras secuencialmente para planificación curricular
   - Verifican asignaciones de coordinadores página por página
   - Necesitan avanzar progresivamente para análisis completo de oferta académica
   - Evalúan distribución de carreras por facultades secuencialmente
...
Los demás escenarios son importantes para robustez (Previous, página específica, casos límite) y características avanzadas (filtros, indicadores), pero el Escenario 3 es el que garantiza que el sistema cumple su propósito fundamental: **permitir a la Autoridad navegar eficientemente hacia adelante por bloques manejables de carreras para gestionar grandes volúmenes de información académica de manera efectiva, que es la funcionalidad core y el flujo de uso más frecuente de la paginación para gestión de la oferta académica institucional**.